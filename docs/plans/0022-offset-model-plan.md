...
이 문서는 canonical square offset regression을 `offset`이라는 새 model로 추가하는 작업을 기록한다.
...
`offset` model은 backbone과 spatial 정보를 보존하는 coordinate head로 구성되어 고정된 canonical
square의 네 vertex에 대한 offset 8개를 회귀한다. 이는 DeTone 계열의 4-point parametrization으로
homography와 정보량이 동등하며, 새로운 localization 방법을 비교하는 저비용 regression baseline 역할을
한다.

이 작업의 목표는 `offset`을 현재 repository의 통일된 model pattern에 맞게 추가하는 것이다. `reg` model은
이미 `CoordSpatialHead`와 통일된 backbone stack을 재사용하므로 `offset`은 사실상 canonical-offset
parametrization만 다른 `reg`다. 실제로 새로 작성하는 logic은 preprocessor에서 canonical square를 빼는 것,
postprocessor에서 `alpha*tanh`와 canonical square를 더하는 것, loss 경로를 postprocessor와 일치시키는
`compute_losses` override뿐이다.
...
- reprojection auxiliary loss와 `estimate_homography`는 포함하지 않는다. 현재 metric set에는
  reprojection-error metric이 없으므로 offset에 대한 단순 smooth L1로 유지한다.
...
설계 요소를 현재 구현 구성으로 다음과 같이 대응한다.

| 설계 요소 | offset 구현 |
| --- | --- |
| canonical square 대비 offset 회귀 | `OffsetModel(BaseModel)`: `CustomBackbone`/`TorchBackbone`/`TimmBackbone` + `CoordSpatialHead` 재사용, `reg`처럼 `--network`/`--head`로 선택 |
| `--backbone` selector | `--network`/`--net` selector와 `--head spatial` |
| discriminative-LR AdamW | two-phase warmup `build_optimizer`/`build_scheduler` |
...
two-phase discriminative-LR warmup을 그대로 사용한다. 기본 loss는 offset 전용 smooth L1이다. 현재
...
smooth L1인 `OffsetSmoothL1Loss(BaseLoss)`를 정의하여 공용 component를 수정하지 않는다. metric은
`PolygonIoU()`다. `compute_losses(raw_output, targets)`는
