...
| 세그멘테이션 신경망 $f_\theta$ (MobileNetV3 + U-Net) | `HybridModel`. backbone은 MobileNetV3(경량성이 설계 목표)로 고정하거나 기존 backbone 선택 로직에서 mobilenet 계열을 기본값으로 사용. 디코더는 `src/components/decoders.py`의 `UNetDecoder`(U-Net 스타일) 재사용, 최종 1채널 head |
...
- backbone(`network`): 경량성이 설계 목표이므로 기본값을 `mobilenet_v3_large`로 한다. 구현 시점에 `src/components/backbones.py`에 mobilenet 계열이 없었으므로, torchvision `mobilenet_v3_large`를 `TorchBackbone`에 정식 추가했다(가중치 경로 `/mnt/d/backbones/mobilenet_v3_large-8738ca79.pth`, stage index `(1, 3, 6, 12, 16)`, stage channel `(16, 24, 40, 112, 960)`, stage stride `(2, 4, 8, 16, 32)`). 이는 backbone 목록에 항목을 추가하는 순수 가산 변경으로 기존 model 동작에 영향을 주지 않는다. `efficientnet`와 동일한 stage 추출 경로를 공유한다.
- 디코더: `UNetDecoder`(U-Net 스타일 additive/skip 업샘플링)를 재사용하고 최종 1채널 마스크 head를 둔다. `seg` model의 head 구조와 동일하게 맞춘다.
...
- `src/models/hybrid/model.py`: `HybridModel` 신규 작성. MobileNetV3 인코더 + `UNetDecoder` + 1채널 마스크 head. `forward`는 `(N, 1, H, W)` 마스크 logits를 반환한다(고전 CV는 model이 아니라 postprocessor에 위치).
...
- `BasePostprocessor` 시그니처 확장(원본 이미지 전달)은 다른 method에 영향을 주는 변경이므로 이 플랜에서 하지 않는다. cornerSubPix 그래디언트 입력은 원본 이미지 대신 예측 확률 지도(sigmoid map)를 사용한다(아래 "확인 결과" 참조).
...
- import 검증: `PYTHONPATH=<project-root> python -c "import src.core.factory; import src.models.hybrid.wrapper"` 오류 없음.
- 단위 검증: `(2, 3, 224, 224)` 이미지 배치로 `HybridModel(network="mobilenet_v3_large").forward`가 `(2, 1, 112, 112)` 마스크 logits를 반환했다(`mask_stride=2`). `HybridPreprocessor(112)`가 `[0, 1]` 범위의 `(2, 1, 112, 112)` 마스크를 생성했고 `BCEDiceLoss`가 스칼라 손실과 backward를 정상 계산했다.
- 후처리 검증: 정답 코너로 만든 이상적 마스크 logits를 `HybridPostprocessor`에 넣었을 때 고전 CV 연쇄가 원래 코너를 최대 오차 약 0.012(112 해상도에서 약 1픽셀) 이내로 복원했다.
- 통합 검증: `get_wrapper(model="hybrid", network="mobilenet_v3_large", head="hybrid", warmup_epochs=1)`로 생성한 wrapper가 warmup freeze/unfreeze $\to$ `train_step` $\to$ `eval_step` $\to$ `predict_step`을 오류 없이 수행했고, `iou`와 `sr`(유효 예측 비율 1.0) 지표를 집계했으며 `predict_step`이 `(2, 4, 2)`를 반환했다.
- 기존 model 회귀: mobilenet backbone 추가 후에도 `SegModel`/`RegModel`의 `custom` network 빌드와 forward가 그대로 동작했다.
- 실제 학습 스크립트 실행(`scripts/train.py --model hybrid --network mobilenet_v3_large --head hybrid`)과 후처리 파라미터 격자 탐색, `PolygonIoU`/SR 최종 수치는 이 플랜의 범위 밖이며 후속 작업에서 확인한다.
