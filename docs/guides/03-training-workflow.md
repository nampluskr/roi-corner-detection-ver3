...
automatic experiment name에는 이 값이 모두 들어가지 않는다. seed나 CSV가 다르지만 model, batch size,
epoch, network, head가 같으면 같은 directory가 된다. 중요한 실험은 다음처럼 사람이 구분할 수 있는
`--output_dir`을 사용한다.

```text
outputs/public/reg/custom_gap/smoke_seed42/
outputs/public/reg/custom_gap/full_seed42/
```
...
python scripts/train.py \
  ...
  --num_workers 0 \
  --output_dir outputs/public/reg/custom_gap/smoke_seed42 \
  --save
```
...
python scripts/evaluate.py \
  ...
  --num_workers 0 \
  --checkpoint outputs/public/reg/custom_gap/smoke_seed42/model.pth \
  --output_dir outputs/public/reg/custom_gap/smoke_seed42
```
...
python scripts/train.py \
  ...
  --output_dir outputs/public/seg/resnet18_mask/smoke_seed42 \
  --save
```
...
python scripts/train.py \
  ...
  --num_workers 4 \
  --output_dir outputs/public/seg/resnet18_mask/full_seed42 \
  --save
```
...
python scripts/evaluate.py \
  ...
  --checkpoint outputs/public/seg/resnet18_mask/full_seed42/model.pth \
  --output_dir outputs/public/seg/resnet18_mask/full_seed42
