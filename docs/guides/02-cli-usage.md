...
| 평가 | `python scripts/evaluate.py` | checkpoint (`--checkpoint` 또는 stage `model.pth`) | `metrics.json` |
| 예측 기록 | `python scripts/predict.py` | checkpoint (`--checkpoint` 또는 stage `model.pth`) | `predictions.csv` |

evaluation과 prediction은 `--checkpoint`를 생략하면 현재 stage output 경로의 `model.pth`를 기본 checkpoint로
사용하며, 그 파일이 없으면 `FileNotFoundError`를 발생시킨다. training은 `--checkpoint`를 load하지 않으며,
해당 option을 주면 저장 위치로만 사용한다. training은 `--dataset` 값에 따라 이전 단계 `model.pth`가 있으면
그 weight를 자동으로 불러와 학습을 시작한다.
...
| `--train_size` | integer | `5000` | split 이후 train sample limit |
...
| `--head` | `spatial` | model별 output variant |
...
`scripts/train.py`는 `--dataset` 값에 따라 이전 단계 weight를 자동으로 초기화한다. 학습 시작 전에 현재
`--dataset`의 이전 단계 output 경로에 있는 `model.pth`를 찾아 있으면 model weight를 불러온다. 단계 선후
관계는 `synthetic`의 이전 단계가 `public`, `measured`의 이전 단계가 `synthetic`이며, `public`이거나 이전
단계 `model.pth`가 없으면 backbone init에서 학습하고 그 사실을 log에 남긴다. 이 초기화는 optimizer와
scheduler 상태를 이어받지 않는다.
...
| `--checkpoint` | 없음 | 평가와 예측의 load path, 학습의 선택적 save path. 평가와 예측에서 생략하면 stage `model.pth`를 사용 |
...
global parser default는 `reg/custom/spatial`에 바로 맞는다. 다른 model에는 다음과 같이 head를 명시한다.

| model | 대표 head | network 예시 |
| --- | --- | --- |
| `reg` | `spatial`, `gap` | `custom`, `resnet18`, `vit_b_16` |
| `offset` | `spatial`, `gap` | `custom`, `resnet18`, `vit_b_16` |
| `seg` | `mask` | `custom`, `resnet18` |
| `det` | `box`, `point` | `custom`, `resnet18` |
| `peak` | `peak` | `custom`, `resnet18` |
| `ridge` | `ridge`, `peakprod` | `custom`, `resnet18` |
...
위 smoke command에서 public, reg, custom, spatial을 사용하면 path는 다음과 같다.

```text
outputs/public/reg/reg_custom_spatial_public/
```

`exp_name`에 seed, CSV, image size, batch size, epoch, train size, patience, warmup은 들어가지 않는다. 이
값만 다르게 실행하면 같은 directory의 file을 덮어쓸 수 있으므로 중요한 비교 실험에는 `--output_dir`을
명시하는 편이 안전하다.
...
```bash
python scripts/train.py --csv_path /data/gt.csv --model reg --network custom --head spatial --save
...
python scripts/train.py --csv_path /data/gt.csv --model ridge --network custom --head pcaline --save
...
data 전략의 세 단계 `public`, `synthetic`, `measured`를 이어서 학습할 때는 `--dataset`만 단계별로
바꾸면 된다. 같은 model, network, head 조합을 유지하면 다음 단계 학습이 이전 단계 output 경로의
`model.pth`에서 weight를 자동으로 이어받는다.

```bash
python scripts/train.py --csv_path /data/public/gt.csv --dataset public --model reg --network custom --head spatial --save
python scripts/train.py --csv_path /data/synthetic/gt.csv --dataset synthetic --model reg --network custom --head spatial --save
python scripts/train.py --csv_path /data/measured/gt.csv --dataset measured --model reg --network custom --head spatial --save
```

`public` 단계는 이전 단계가 없으므로 backbone init에서 학습한다. `synthetic`과 `measured` 단계는 각각
`public`과 `synthetic` 단계의 `model.pth`에서 weight를 불러온다. 이전 단계 `model.pth`가 없으면 error가
아니라 backbone init으로 fallback하고 log를 남기므로, 이어서 학습하려면 앞 단계에서 `--save`로 checkpoint를
남겨야 한다. 단계마다 learning rate를 줄이는 스케줄은 자동 적용되지 않으므로 필요하면
[Data Strategy](../architecture/05-data-strategy.md)의 설계 원칙에 따라 별도로 조정한다.
...
python scripts/evaluate.py \
  ...
  --num_workers 0 \
  --checkpoint outputs/public/reg/reg_custom_spatial_public/model.pth \
  --output_dir outputs/public/reg/reg_custom_spatial_public
```
...
batch size와 `max_epochs`는 일반적으로 inference 결과의 의미를 바꾸지 않고 default output path에도
포함되지 않는다. 결과 위치 혼동을 막으려면 학습 값을 유지하거나 `--output_dir`을 명시한다.

## 12. `--checkpoint`의 작업별 의미

동일한 option 이름이지만 script별 의미가 다르다.

| script | checkpoint가 있을 때 | 없을 때 |
| --- | --- | --- |
| `train.py` | 학습 후 해당 path에 저장, `--save`도 필요 | default output 아래 저장, `--save`도 필요. 이전 단계 `model.pth`는 자동 초기화 |
| `evaluate.py` | 실행 전에 weight load | stage `model.pth` load, 그 파일도 없으면 오류 |
| `predict.py` | 실행 전에 weight load | stage `model.pth` load, 그 파일도 없으면 오류 |

current training script에는 checkpoint를 load해 optimizer와 epoch를 이어가는 resume 기능이 없다. 학습은
`--dataset`에 따라 이전 단계 `model.pth`가 있으면 그 weight로 자동 초기화하며, 이는 model weight만 불러오고
optimizer와 epoch는 이어가지 않는다.
...
python scripts/train.py \
  ...
  --output_dir outputs/public/reg/seed42_full
```
...
| head 관련 `ValueError` | 선택 model과 head 불일치 | model별 head 명시 |
...
| output이 예상한 곳에 없음 | dataset, model, network, head 불일치 | calculated path 또는 `--output_dir` 확인 |
...
| stage checkpoint 경로 | `scripts/config.py` |
| train command와 이전 단계 자동 초기화 | `scripts/train.py` |
...
관리하고, 평가와 예측에서 학습 당시 값을 다시 지정해야 한다. 평가와 예측에서 `--checkpoint`를 생략하면
해당 stage의 `model.pth`를 사용하고, 학습은 `--dataset`에 따라 이전 단계 `model.pth`가 있으면 그
weight로 자동 초기화한다. 첫 실행은 명시적 CSV, image size 224, 작은 subset, worker 0으로 확인한 뒤
규모를 늘리는 것이 좋다.
