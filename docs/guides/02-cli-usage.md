...
바꾸면 된다. 같은 model과 network, head, experiment 조합을 유지하면 다음 단계 학습이 이전 단계 output
경로의 `model.pth`에서 weight를 자동으로 이어받는다.

```bash
python scripts/train.py --csv_path /data/public/gt.csv --dataset public --model reg --network custom --head gap --save
python scripts/train.py --csv_path /data/synthetic/gt.csv --dataset synthetic --model reg --network custom --head gap --save
python scripts/train.py --csv_path /data/measured/gt.csv --dataset measured --model reg --network custom --head gap --save
```
