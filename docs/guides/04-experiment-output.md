...
한다. `evaluate.py`와 `predict.py`에서 `--checkpoint`를 생략하면 현재 stage output directory의 `model.pth`를
default로 load하며, 그 파일이 없으면 `FileNotFoundError`가 발생한다.

`model.pth`는 staged 학습의 입력이기도 하다. `train.py`는 이전 단계 output의 `model.pth`가 있으면 그
state dictionary를 불러와 학습을 시작한다. optimizer와 epoch는 저장되지 않으므로 이는 resume이 아니라
weight 초기화이며, 이전 단계 `model.pth`가 없으면 error 없이 backbone init에서 학습한다.
