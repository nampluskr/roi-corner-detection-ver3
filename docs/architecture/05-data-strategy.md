...
path의 logical stage 이름으로 사용되며 세 단계를 하나의 명령으로 자동 orchestrate하는 별도 pipeline은
현재 공통 CLI에 포함되지 않는다. 대신 `scripts/train.py`는 `--dataset` 값에 따라 이전 단계에서 학습된
weight를 자동으로 이어받는 staged 학습을 지원한다.
...
- `scripts/train.py`는 이전 단계 `model.pth`가 있으면 그 weight를 자동으로 불러와 staged 학습의 weight
  carryover를 지원하고, 없으면 backbone init으로 fallback한다. 세 단계를 하나의 명령으로 자동
  orchestrate하고 단계별 learning rate를 자동 조정하는 script는 현재 공통 CLI 범위 밖이다.
- 합성 fringe 생성은 개념 설계이며 `scripts/train.py`가 자동 생성하지 않는다.
