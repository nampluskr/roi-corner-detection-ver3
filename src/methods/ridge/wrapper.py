# src/methods/ridge/wrapper.py: composes RidgeModel/RidgePreprocessor/RidgePostprocessor and focal loss

from torch.optim import AdamW
from torch.optim.lr_scheduler import ReduceLROnPlateau

from src.methods.base.wrapper import BaseWrapper
from src.methods.ridge.model import RidgeModel
from src.methods.ridge.preprocessor import RidgePreprocessor
from src.methods.ridge.postprocessor import RidgePostprocessor
...
class RidgeWrapper(BaseWrapper):
    ...
    def __init__(self, in_channels=3, backbone="custom", head="ridge", image_size=224,
                 optimizer=None, scheduler=None, preprocessor=None, postprocessor=None,
                 losses=None, metrics=None, device=None, warmup_epochs=1):
        if head not in (None, "ridge"):
            raise ValueError("Unknown ridge head: %s. Supported: ridge" % head)
        model = RidgeModel(in_channels=in_channels, backbone=backbone)
        preprocessor = preprocessor or RidgePreprocessor(image_size // model.ridge_stride)
        postprocessor = postprocessor or RidgePostprocessor()
