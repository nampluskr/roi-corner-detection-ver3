# src/methods/ridge/model.py: stage-based composable ridge model for corner localization
...
from src.components.heads import HeatmapHead
...
class RidgeModel(BaseModel):
    ...
    def __init__(self, in_channels=3, backbone="custom", upsample="interpolate_conv"):
        ...
        elif backbone in TORCH_RIDGE_BACKBONES:
            encoder = TorchBackbone(backbone)
        elif backbone in TIMM_CNN_BACKBONES:
            encoder = TimmBackbone(backbone)
        else:
            raise ValueError("Unknown ridge backbone: %s. Supported: %s"
                             % (backbone, ", ".join(SUPPORTED_RIDGE_BACKBONES)))
