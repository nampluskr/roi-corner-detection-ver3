# src/models/torchseg/model.py: torchvision whole-model segmentation adapted to binary mask logits
...
class TorchSegModel(BaseModel):
    ...
    def __init__(self, network="fcn_resnet50", pretrained=True):
        super().__init__()
        network = network or "fcn_resnet50"
        if network not in TORCHSEG_BUILDERS:
            raise ValueError("Unknown torchseg network: %s. Supported: %s"
                             % (network, ", ".join(SUPPORTED_TORCHSEG_MODELS)))

        self.model_name = network
        self.net = self.build_model(network, pretrained=pretrained)
        self.mask_stride = 1
