# src/models/gcn/wrapper.py: composes GCNModel/GCNPreprocessor/GCNPostprocessor and deep-supervised SmoothL1
...
from src.components.losses import DeepSupervisedSmoothL1Loss
from src.components.metrics import PolygonIoU


class GCNWrapper(BaseWrapper):
    """Wraps gcn models behind the shared Trainer/Evaluator/Predictor interface."""

    def __init__(self, in_channels=3, network="custom", head="gcn", image_size=224,
                 optimizer=None, scheduler=None, preprocessor=None, postprocessor=None,
                 losses=None, metrics=None, device=None, warmup_epochs=1):
        if head not in (None, "gcn"):
            raise ValueError("Unknown gcn head: %s. Supported: gcn" % head)
        model = GCNModel(in_channels=in_channels, network=network)
        ...
        self.set_losses(self.losses or {"loss": DeepSupervisedSmoothL1Loss()})
