...
| 표현 | 핵심 질문 | 해당 model |
| --- | --- | --- |
| coordinate | 좌표를 바로 예측할 수 있는가 | `reg`, `offset` |
...
| `offset` | canonical square 대비 corner offset 회귀 | `spatial`, `gap` | `(B, 8)` offsets |
...
| `ridge` | edge Gaussian ridge map | `ridge`, `peakprod` | 4-channel logits |
...
앞의 8개는 project component를 조립하는 composable model이고 뒤의 4개는 external whole-model이다.
...
## 페이지 15. offset (Canonical Offset Regression)

고정된 canonical square의 네 vertex 대비 corner offset 8개를 회귀한다. DeTone 계열 4-point
parametrization으로 homography와 정보량이 동등한 기존 방법론 baseline이다.
...
flowchart LR
  ...
  B --> C[CNN or transformer adapter]
  C --> D["global or spatial feature"]
  D --> E["coordinate head (spatial or gap)"]
  E --> F["raw offsets (B,8)"]
  F --> G["alpha*tanh + canonical + clamp"]
  G --> H["final corners (B,4,2)"]
  I["corner target (B,4,2)"] --> J["preprocessor: subtract canonical, reshape to (B,8)"]
  J --> K["OffsetSmoothL1Loss"]
  F --> L["alpha*tanh offsets (B,8)"]
  L --> K
```

후처리 결과 예시는 다음과 같다.

![offset postprocess](assets/postprocess_offset.png)

| 요소 | 내용 |
| --- | --- |
| 전처리 | `(B,4,2)`에서 canonical vertex를 빼고 `(B,8)` offset으로 reshape |
| 후처리 | `alpha*tanh` 후 canonical vertex를 더하고 `clamp(0,1)`, threshold나 NMS 없음 |
| 손실함수 | `OffsetSmoothL1Loss`, `beta=1`, offset과 target을 같은 `alpha*tanh` 경로로 비교 |
| 특징 | `MARGIN=0.25` canonical square, `ALPHA=0.25` 변위 제한, 작은 변위 prior로 초기 수렴 안정 |

## 페이지 16. seg (Binary ROI Segmentation)
...
## 페이지 17. peak (Dense Corner Heatmap)
...
![ridge peakprod postprocess](assets/postprocess_ridge_peakprod.png)
...
## 페이지 20. gcn (Iterative Graph Refinement)
...
## 페이지 21. hybrid (Learned Mask + Classical Geometry)
...
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `reg` | `custom` | `gap` |  |  |  |  |  |  |
| `offset` | `custom` | `spatial` |  |  |  |  |  |  |
| `seg` | `resnet18` | `mask` |  |  |  |  |  |  |
| `peak` | `custom` | `peak` |  |  |  |  |  |  |
| `ridge` | `custom` | `ridge` |  |  |  |  |  |  |
| `ridge` | `custom` | `peakprod` |  |  |  |  |  |  |
| `det` | `custom` | `box` |  |  |  |  |  |  |
| `gcn` | `custom` | `gcn` |  |  |  |  |  |  |
...
- 12개 model registry와 CLI 조립 축 확립
- 후처리 시각화 도식 자산 생성
...
- 페이지 24 비교 표와 후처리 이미지를 실제 예측 결과로 교체
